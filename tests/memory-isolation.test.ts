...
describe('Persona Memory Isolation', () => {
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'symbiont-isolation-'))
    db = new MemoryDB(tempDir)
  })

  it('should add card with default owner xiaoxi', () => {
    const card = db.addCard({
      content: 'test card', scene: 'test', tags: ['test'],
      confidence: 0.7, source: [], connections: [], owner: 'xiaoxi',
    })
    assert.strictEqual(card.owner, 'xiaoxi')
  })
  ...
  it('scope=self: only returns own cards', () => {
    db.addCard({ content: 'xiaoxi card', scene: 'test', tags: ['a'], confidence: 0.7, source: [], connections: [], owner: 'xiaoxi' }, 'xiaoxi')
    ...
    const results = db.searchCards({ scope: 'self', owner: 'xiaoxi' })
    assert.strictEqual(results.length, 1)
    assert.strictEqual(results[0].content, 'xiaoxi card')
  })
  ...
  it('scope takes priority over owner', () => {
    db.addCard({ content: 'xiaoxi card', scene: 'test', tags: [], confidence: 0.7, source: [], connections: [], owner: 'xiaoxi' }, 'xiaoxi')
    db.addCard({ content: 'shared card', scene: 'test', tags: [], confidence: 0.7, source: [], connections: [], owner: 'shared' }, 'shared')

    // scope=shared should override owner=xiaoxi
    const results = db.searchCards({ scope: 'shared', owner: 'xiaoxi' })
    assert.strictEqual(results.length, 1)
    assert.strictEqual(results[0].owner, 'shared')
  })

  it('no scope, no owner: returns all cards (backward compat)', () => {
    db.addCard({ content: 'card1', scene: 'test', tags: [], confidence: 0.7, source: [], connections: [], owner: 'xiaoxi' }, 'xiaoxi')
  ...
  it('updateCard can change owner to shared', () => {
    const card = db.addCard({ content: 'personal insight', scene: 'test', tags: ['insight'], confidence: 0.7, source: [], connections: [], owner: 'xiaoxi' }, 'xiaoxi')
    assert.strictEqual(card.owner, 'xiaoxi')
  ...
  it('getAllEmbeddings filters by owner', () => {
    ...
    const xAll = db.getAllEmbeddings('xiaoxi')
  ...
  it('keyword search respects scope', () => {
    db.addCard({ content: 'proxy settings tip', scene: 'deploy', tags: ['docker'], confidence: 0.7, source: [], connections: [], owner: 'xiaoxi' }, 'xiaoxi')
    db.addCard({ content: 'proxy config shared', scene: 'deploy', tags: ['docker'], confidence: 0.7, source: [], connections: [], owner: 'shared' }, 'shared')

    const selfResults = db.searchCards({ keyword: 'proxy', scope: 'self', owner: 'xiaoxi' })
    assert.strictEqual(selfResults.length, 1)
    assert.strictEqual(selfResults[0].owner, 'xiaoxi')
